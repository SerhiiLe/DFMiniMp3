...
{
public:
	explicit DFMiniMp3(typename T_NOTIFICATION_METHOD::TargetType* target = nullptr) :
	...
	bool begin(Stream &stream, bool doReset = true, unsigned int timeout = 2000)
	{
		_serial = &stream;
	...
	*/
	...
	void reset(bool waitForOnline = true, unsigned int timeout = 2000)
	{
		...
		while (waitForOnline && !_isOnline && millis() - start < timeout)
		{
			delay(1);
			loop();
		}
